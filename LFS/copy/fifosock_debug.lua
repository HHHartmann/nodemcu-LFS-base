...
return function(sock)
  ...
  local ssend  = function(s,islast)
    ...
    if type(s) == "function" then
      ...
      s, ns = s()
      print("calling function returned", s, ns)
    elseif type(s) == "string" and sslan < COALIMIT then
      print("sending", type(s), s)
      if sslan == 0
       then ssla, sslan = s, 1
  ...
  return function(s)
    ...
    if type(s) == "function" then       print("queuing function", s)  promote(); fbig:queue(s, ssend); return; end
    ...
    while #s > BIGTHRESH + SPLITSLOP do
      print("Splitting big string size:", #s)
     local pfx
     pfx, s = s:sub(1,BIGTHRESH), s:sub(BIGTHRESH+1)
     fbig:queue(pfx, function(t) print("corking==================================") corked = true; return t end)
    end
